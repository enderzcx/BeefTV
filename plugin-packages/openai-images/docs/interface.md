...
| `create.files[0].source` | `{"$filter":{"from":{"$sortByOrder":{"$ref":"request.images"}},"as":"media","where":{"$ne":[{"$ref":"media.role"},"mask"]}}}` |
| `create.files[0].filename` | `"image.png"` |
| `create.files[1].name` | `"mask"` |
| `create.files[1].source` | `{"$first":{"$filter":{"from":{"$sortByOrder":{"$ref":"request.images"}},"as":"media","where":{"$eq":[{"$ref":"media.role"},"mask"]}}}}` |
...
无参考图走 JSON generations；有参考图或蒙版走 multipart edits，参考图以 image 文件部件上传，蒙版以 mask 文件部件上传。quality 的 1k/2k/4k 映射为 OpenAI low/medium/high。
