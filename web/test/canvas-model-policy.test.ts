...
function policyConfig(): AiConfig {
    ...
    const channel: ModelChannel = {
        ...
        modelProfiles: variants.map((item) => {
...
describe("逻辑模型选择", () => {
    test("技能上下文展开后按最终视频模型字符上限拒绝提交", () => {
        ...
        config.channels[0]!.modelProfiles![0]!.capabilityConfig!.video!.references.promptMaxChars = 10_000;
    ...
    test("后台图片默认比例优先于旧的全局 1:1 配置", () => {
        ...
        const config: AiConfig = {
            ...defaultConfig,
            size: "1:1",
            channels: [{ id: "platform", name: "平台模型", baseUrl: "/api", apiKey: "system", apiFormat: "openai", scope: "system", models: ["managed-image"], modelProfiles: [{ model: "managed-image", capability: "image", billingMode: "fixed_request", unitPriceMicrocredits: 1, logicalModelId: "managed-image", logicalCapabilitySpec: { version: 1, capability: "image" }, capabilityConfig: profile }] }],
            models: [model],
            imageModels: [model],
            imageModel: model,
            model,
    ...
    test("渠道视频能力配置优先于旧的全局 6 秒和 1:1 配置", () => {
        ...
        const config: AiConfig = {
            ...
            videoSeconds: "6",
            size: "1:1",
            channels: [{ id: "autodl-channel", name: "AutoDL", baseUrl: "https://autodl.art", apiKey: "system", apiFormat: "openai", scope: "system", models: ["MiniMax H3"], modelProfiles: [{ model: "MiniMax H3", capability: "video", protocol: "autodl-comfyui", billingMode: "fixed_request", unitPriceMicrocredits: 1, capabilityConfig: profile }] }],
            models: [model],
            videoModels: [model],
            videoModel: model,
            model,
    ...
    test("不支持画幅的视频模型不会继续提交旧的全局尺寸", () => {
        ...
        const config: AiConfig = {
            ...
            size: "16:9",
            vquality: "720",
            channels: [{ id: "autodl-channel", name: "AutoDL", baseUrl: "https://autodl.art", apiKey: "system", apiFormat: "openai", scope: "system", models: ["minimax_h3_lightx2v_no_pic"], modelProfiles: [{ model: "minimax_h3_lightx2v_no_pic", capability: "video", protocol: "autodl-comfyui", billingMode: "fixed_request", unitPriceMicrocredits: 1, capabilityConfig: profile }] }],
            models: [model],
            videoModels: [model],
            videoModel: model,
            model,
    ...
    test("图片加音频可匹配支持图生视频和参考音频的模型", () => {
        ...
        const imageCost = config.channels[0]?.modelProfiles?.find((item) => item.model === "cinema-image");
...
describe("图片参考图上限", () => {
    test("超出当前模型上限时保留全部输入并返回明确错误", () => {
        const config = policyConfig();
        const modelProfile = config.channels[0]?.modelProfiles?.[0];
        if (!modelProfile?.capabilityConfig?.image) throw new Error("缺少图片能力配置");
        modelProfile.capabilityConfig.image.references.maxImages = 1;
        const references = [
