...
// Keep generated modules under the workspace so Bun's test loader can resolve
// them consistently on macOS paths that contain non-ASCII parent directories.
const dir = mkdtempSync(join(import.meta.dir, ".agent-reliability-"));
const root = new URL("../src/", import.meta.url);
const requestPath = join(dir, "request.ts");
const timerPath = join(dir, "timer.ts");
...
writeFileSync(timerPath, 'export const delays: number[] = []; export const testSetTimeout = (fn: TimerHandler, ms?: number, ...args: unknown[]) => { delays.push(ms ?? 0); return globalThis.setTimeout(fn, Math.min(ms ?? 0, 1), ...args); }; export const testClearTimeout = (timer: ReturnType<typeof setTimeout>) => globalThis.clearTimeout(timer);');
writeFileSync(join(dir, "agent.ts"), `import { testSetTimeout, testClearTimeout } from ${JSON.stringify(pathToFileURL(timerPath).href)};\n${readFileSync(new URL("services/api/agent.ts", root), "utf8")}`
    .replace('"@/services/api/request"', JSON.stringify(pathToFileURL(requestPath).href))
    .replace('"@/services/api/task-text-stream"', JSON.stringify(pathToFileURL(fileURLToPath(new URL("services/api/task-text-stream.ts", root))).href))
    .replaceAll("setTimeout(", "testSetTimeout(")
    .replaceAll("clearTimeout(", "testClearTimeout("));
...
function observe(options: { timeoutMs?: number; after?: number } = {}) {
    ...
    const done = new Promise<unknown>((resolve, reject) => {
        const deadline = globalThis.setTimeout(() => reject(new Error("subscription did not settle")), 3000);
...
describe("Agent SSE recovery", () => {
    ...
    it.each(["", snapshot("running")])("bounds clean EOF retries even with repeated snapshots", async (body) => {
        ...
        // Ignore the idle watchdog; only the four bounded reconnect waits are
        // part of the retry budget.
        expect(timers.delays.filter((ms) => ms < 45_000)).toHaveLength(4);
    });
