    ...
    for (const approvedBeforeDisconnect of [false, true]) test(`重做批准${approvedBeforeDisconnect ? "已落盘但回执丢失" : "尚未落盘"}，重新装载后仅准备原目标并保留参考节点`, async () => {
        ...
        const api: typeof creationRuns = { ...creationRuns,
            ...
            prepare: async (_id, input) => {
                prepared.push(input);
                const submission: CreationSubmission = { id: "new-b", runId: "run", itemKey: input.itemKey, requestHash: "hash", execution: { model, configHash: "execution" } };
                submissions.push(submission); return submission;
            },
            approve: async (_id, input) => ({ submissions: submissions.filter((item) => input.submissionIds.includes(item.id)).map((item) => Object.assign(item, { approvedAt: "2026-01-01" })) }),
            execute: async () => { executions++; return { id: "task-new-b", status: "queued" } as GenerationTask; },
            canvasSnapshot: async () => ({ document: { id: "canvas", title: "画布", nodes: snapshot.nodes, connections: snapshot.connections, chatSessions: [], activeChatId: null, viewport: snapshot.viewport, createdAt: "", updatedAt: "", directorScenes: [] }, snapshotHash: "before" }),
            commitCanvas: async () => ({ snapshotHash: "after" }),
        };
        const create = () => new CreativeAgentController({ config: () => config, canvas: () => ({ canvasId: "canvas", read: () => snapshot, apply: async () => snapshot }), onChange: () => undefined, onOpenCanvas: () => undefined, api, queryTask: async () => ({ status: "succeeded" } as GenerationTask), waitTask: async () => ({ id: "task-new-b", status: "succeeded", resultJson: JSON.stringify({ images: [{ storageKey: "resource:new-b" }] }) } as GenerationTask), ensureAsset: async () => ({ assetId: "asset-new-b", created: true, linkedToProject: true }) });
