...
test("media task submission refreshes the canvas so the generating node is visible immediately", async () => {
    const refresh = mock(async () => {});
    const sync = createAgentCanvasSync({ canvasId: "canvas", applyPatches: async () => {}, refresh, onError: () => {}, batchMs: 0 });
    sync.receive(event("generation_task_created", { canvasId: "canvas", toolName: "generate_media", taskId: "task-1", nodeId: "video-1" }));
    await sleep();
    sync.dispose();
    expect(refresh).toHaveBeenCalledTimes(1);
});
