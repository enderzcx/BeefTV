...
describe("canvas starter", () => {
    test("opens a new canvas in the LibTV-style freeform entry", () => {
        expect(resolveCanvasEmptyStateKind({ nodeCount: 0, shortDramaEnabled: true, isProjectLinked: false })).toBe("freeform");
    });
    test("keeps the guided entry opt-in for explicit short-drama starts", () => {
        expect(resolveCanvasEmptyStateKind({ nodeCount: 0, shortDramaEnabled: true, isProjectLinked: false, starterMode: "guided" })).toBe("guided");
    });
