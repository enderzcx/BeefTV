...
describe("创作方案执行合同", () => {
    ...
    test("不能静默缩短已确认时长，也不能忽略模型输入上限", () => {
        ...
        const limited = structuredClone(config); limited.channels[0]!.modelProfiles![1]!.capabilityConfig!.video!.references.maxImages = 1;
