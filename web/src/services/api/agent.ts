...
export type CreateAgentRunInput = {
    ...
    permissionMode?: AgentPermissionMode;
    contextScope?: string[];
    budget?: { maxGenerationTasks?: number; maxVideoSeconds?: number; maxSteps?: number };
...
export function subscribeAgentEvents(runId: string, onEvent: (event: AgentEvent) => void, options: { after?: number; onError?: (error: unknown) => void; onConnectionChange?: (status: "connecting" | "connected" | "reconnecting" | "disconnected") => void; timeoutMs?: number } = {}) {
    ...
    void (async () => {
        ...
        while (!controller.signal.aborted) {
            ...
            // Local model/API startup can legitimately take longer than the
            // hosted path. The run is durable, so a generous idle watchdog is
            // safer than aborting a healthy SSE connection mid-turn.
            const touch = () => { clearTimeout(watchdog); watchdog = setTimeout(abortAttempt, options.timeoutMs ?? 120_000); };
            touch();
                    ...
                    while (!controller.signal.aborted && !attempt.signal.aborted) {
                        ...
                        consumeTaskTextStream(parser, decoder.decode(value, { stream: !done }), (item) => {
                            if (item.event === "agent_event") {
                                ...
                                const statusPayload = { status: run.status, revision: run.revision, cleanupPending: run.cleanupPending, failureMessage: run.failureMessage, skills: run.skills, step: run.step, approval: run.approval };
