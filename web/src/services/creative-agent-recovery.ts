...
export async function recoverCreativeResponse(initial: Reply, messages: ResponseInputMessage[], context: Context,
    ...
    for (let attempt = 0; attempt <= 2; attempt++) {
        ...
        protocol = [...protocol,
            ...
            { role: "user", content: "这是程序校验反馈，不是用户修改需求。未创建节点或提交媒体生成。请自行修复参数，保留已知要求与真实引用；不让用户填写技术字段。缺少真实素材、事实或能力时，用creative_respond.questions主动提问，选项来自当前上下文且允许自由补充。不再仅说‘已修复’或‘请确认’，必须返回可校验的完整方案或可回答问题。仍无法组成媒体方案时可先提供纯文字创意并提问下一步，不虚构媒体配置。" },
        ];
