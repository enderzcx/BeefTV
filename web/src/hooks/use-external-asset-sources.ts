...
export function useExternalAssetSources(open: boolean): ExternalAssetSourceState {
    ...
    // Local workspaces must not even enumerate external plugin sources: listing
    // folders/items is already a cloud/SaaS read, even when auto-upload is off.
    const sources = useMemo(() => localMode ? [] : createHostedAssetSources(hydrated ? installations : []), [hydrated, installations, localMode]);
