...
const permissionOptions: Array<{ value: AgentPermissionMode; label: string; description: string; icon: typeof ShieldCheck; color: string }> = [
    ...
    { value: "auto", label: "自动执行", description: "按运行限制执行已授权工具", icon: Sparkles, color: "#429477" },
];
        ...
        <div className="canvas-agent-settings-scroll thin-scrollbar min-h-0 flex-1 space-y-6 overflow-y-auto p-4">
            ...
            <section>
                ...
                <div className="min-w-0 rounded-xl px-2 py-1.5" style={{ background: theme.node.fill }}>
                    <ModelPicker config={props.config} value={props.selectedModel} capability="text" onChange={props.onModelChange} variant="creation" fullWidth placeholder="选择文本模型" className="!border-0 !bg-transparent !shadow-none" popoverClassName="agent-model-picker-popover" />
                </div>
            </section>
            <section>
            ...
            <section>
                ...
                <div className="space-y-1">
                    ...
                    <SettingRow theme={theme} icon={<Gauge className="size-4" />} title="运行限制" summary={`生成任务 ${props.maxGenerationTasks || "0"} · 视频秒数 ${props.maxVideoSeconds || "0"}（0 表示不限）`} onClick={() => onOpen("budget")} />
                </div>
            </section>
        </div>
    );
...
function BudgetWorkspace({ props, theme }: { props: AgentSettingsProps; theme: CanvasTheme }) {
    return <div className="thin-scrollbar min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5"><div className="rounded-xl p-4" style={{ background: theme.node.fill }}><div className="text-sm font-semibold">本轮运行限制</div><p className="mt-2 text-xs opacity-60">生成任务和视频秒数填 0 表示不设上限；只读模式不执行生成。</p></div><BudgetInput label="生成任务上限" hint="0 表示不限" value={props.maxGenerationTasks} onChange={props.onMaxGenerationTasksChange} theme={theme} /><BudgetInput label="视频秒数上限" hint="0 表示不限；按请求时长累计" value={props.maxVideoSeconds} onChange={props.onMaxVideoSecondsChange} theme={theme} /></div>;
}
...
function sectionTitle(section: SettingsSection) { return section === "profile" ? "长期偏好" : section === "memories" ? "个人记忆" : section === "skills" ? "Skills" : section === "mcp" ? "MCP 与工具" : section === "context" ? "上下文" : "运行限制"; }
function sectionSubtitle(section: SettingsSection) { return section === "profile" ? "用户、项目和画布的长期行为偏好" : section === "memories" ? "只属于你，批准后才会注入会话" : section === "skills" ? "搜索、安装并选择本轮技能" : section === "mcp" ? "本地工具与连接状态" : section === "context" ? "控制 Agent 能读取的范围" : "控制本轮生成任务与视频时长"; }
