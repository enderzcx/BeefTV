    ...
    useEffect(() => {
        ...
        const instance = new CreativeAgentController({
            ...
            onChange: (next) => {
                ...
                const nextDetail: CanvasCreativeDetail = { kind: "creative-interaction", input: original.input, state: next.state, runId: next.run.id, status: next.run.status, approvedProposalVersion: next.run.approvedProposalVersion };
