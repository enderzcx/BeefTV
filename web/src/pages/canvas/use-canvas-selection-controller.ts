...
function snapCanvasOffsetToGrid(offset: Position, anchor: { x: number; y: number }, gridSize: number): Position {
    return {
        x: Math.round((anchor.x + offset.x) / gridSize) * gridSize - anchor.x,
        y: Math.round((anchor.y + offset.y) / gridSize) * gridSize - anchor.y,
    };
}
...
export function useCanvasSelectionController({
    ...
    const handleNodeMouseDown = useCallback((event: ReactMouseEvent | ReactPointerEvent, nodeId: string) => {
        ...
        if (containerRef.current) containerRef.current.dataset.canvasNodeDragging = "true";
        setAlignmentGuides({});
    ...
    const finishNodeDrag = useCallback((clientX?: number, clientY?: number) => {
        ...
        const alignedOffset = calculateNodeAlignment(alignmentContextRef.current, rawOffset, 7 / currentViewport.k).offset;
        const { x: dx, y: dy } = snapToGrid
            ? snapCanvasOffsetToGrid(alignedOffset, initialPositions[0], 16)
            : alignedOffset;

        historyPausedRef.current = false;
        nodeDraggingRef.current = false;
        applyCanvasNodeDragPreview(containerRef.current, null);
        delete containerRef.current?.dataset.canvasNodeDragging;
    ...
    }, [historyPausedRef, nodesRef, onLinkedFolderDrop, onNodeClick, onNodeDragEnd, setNodes, snapToGrid, viewportRef]);
