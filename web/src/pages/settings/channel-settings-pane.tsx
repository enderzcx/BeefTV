...
export function ChannelSettingsPane({ onOpenModels, onOpenRunningHub }: ChannelSettingsPaneProps) {
    ...
    const refreshChannelModels = async (channel: ModelChannel) => {
        ...
        try {
            ...
            updateChannels(latestConfig.channels.map((item) => (item.id === channel.id ? applyFetchedChannelModelCatalog(item, result) : item)), latestConfig);
        ...
        <Form layout="vertical" requiredMark={false}>
            <div className="settings-pane-header">
                ...
                <div className="flex w-full gap-2 sm:w-auto sm:shrink-0">
                    ...
                    <Button className="h-10 flex-1 sm:h-8 sm:flex-none" icon={<Plus className="size-4" />} onClick={addChannel}>新增渠道</Button>
                </div>
            </div>
                        ...
                        return (
                            <section key={channel.id} aria-labelledby={`channel-${channel.id}-title`} className="settings-channel p-2.5 sm:p-3">
                                <div className="mb-2.5 flex flex-wrap items-start justify-between gap-2.5">
									...
									{!builtinBeefAPI ? <Popconfirm title="删除个人模型渠道？" description="该渠道关联的模型选择会同时移除。" okText="删除" cancelText="取消" okButtonProps={{ danger: true }} onConfirm={() => deleteChannel(channel.id)}>
                                ...
                                {editing && (
                                    ...
                                    >
                                        <div className="model-editor-panel">
											...
											<Form.Item label="渠道名称" htmlFor={`channel-${channel.id}-name`} className="mb-0 sm:col-span-1"><Input id={`channel-${channel.id}-name`} value={channel.name} disabled={builtinBeefAPI} placeholder="例如：我的 NewAPI" onChange={(event) => updateChannel(channel.id, { name: event.target.value })} onBlur={(event) => updateChannel(channel.id, { name: event.target.value.trim() || "未命名渠道" })} /></Form.Item>
											<Form.Item label="目录连接类型" className="mb-0 sm:col-span-1" extra={builtinBeefAPI ? "应用内置适配" : "仅影响模型目录拉取。"}><Segmented<UserChannelConnection> block disabled={builtinBeefAPI} value={channelConnectionMode(channel)} options={[{ label: "OpenAI", value: "openai" }, { label: "Gemini", value: "gemini" }]} onChange={(value) => updateChannelConnection(channel, value)} /></Form.Item>
											<Form.Item label="Base URL" htmlFor={`channel-${channel.id}-base-url`} className="mb-0 sm:col-span-1"><Input id={`channel-${channel.id}-base-url`} inputMode="url" value={channel.baseUrl} disabled={builtinBeefAPI} placeholder={localMode ? "填写本地服务 Base URL" : "填写云端渠道 Base URL"} onChange={(event) => updateChannel(channel.id, { baseUrl: event.target.value })} onBlur={(event) => updateChannel(channel.id, { baseUrl: event.target.value.trim().replace(/\/+$/u, "") })} /></Form.Item>
            ...
            ) : <WorkspaceState icon="settings" compact title="暂无个人渠道" action={<Button icon={<Plus className="size-4" />} onClick={addChannel}>新增渠道</Button>} />}
        </Form>
    );
}
...
function ChannelStatus({ channel, persistence }: { channel: ModelChannel; persistence: ModelConfigPersistenceState }) {
	...
	const label = modelConfigChannelStatusLabel(channel, persistence);
    return (
