...
import { createCanvasProjectWithRemoteSync, hasRemoteUserDataSyncSession, saveRemoteUserDataNow } from "@/services/local-workspace-sync";
import { isLocalWorkspaceMode } from "@/services/workspace-mode";

/** Agent 依赖真实画布 ID，先完成服务端保存，再进入同一个画布 Agent。 */
export function CreationAgentEntry({ autoStart = false }: { autoStart?: boolean }) {
    ...
    const start = async () => {
        ...
        try {
            if (!userId || (!localMode && !hasRemoteUserDataSyncSession())) throw new Error("工作区同步尚未就绪，请稍后重试。");
            ...
            if (!created.current) {
                ...
                if (result.syncError && !localMode) throw new Error("画布已缓存在本机，但远端尚未保存。请重试同步后再启动 Agent。");
            } else {
                if (!localMode) await saveRemoteUserDataNow();
            }
    ...
    useEffect(() => {
        if (!autoStart || !hydrated || created.current || lock.current) return;
        void start();
    }, [autoStart, hydrated]);
    return <section className="creation-agent-entry" aria-label="画布 Agent 模式">
        ...
        <div className="creation-agent-actions"><Button type="primary" icon={<Sparkles />} loading={busy} disabled={!hydrated} onClick={() => void start()}>{created.current ? (localWorkspace ? "重新进入画布" : "重试同步并进入") : "新建画布，与 Agent 创作"}<ArrowUpRight /></Button><Button icon={<FolderOpen />} disabled={busy} onClick={() => navigate("/canvas?agent=1")}>在已有画布中继续</Button></div>
