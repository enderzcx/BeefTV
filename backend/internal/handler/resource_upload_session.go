...
func RegisterChunkedUploadRoutes(r *gin.RouterGroup, svc *app.Service, hostedProfile ...bool) {
	includeHostedStorage := true
	if len(hostedProfile) > 0 {
		includeHostedStorage = hostedProfile[0]
	}
	...
	r.POST("/resources/uploads/:id/complete", func(c *gin.Context) {
		...
		if includeHostedStorage {
			...
			resource, svcErr = svc.UploadLocalResourceFile(user.ID, session.FileName, session.Size, session.Kind, session.Width, session.Height, session.DurationMs, fh, session.IdempotencyKey)
		}
