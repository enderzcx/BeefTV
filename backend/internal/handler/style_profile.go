...
func RegisterStyleProfileRoutes(r *gin.RouterGroup, svc *app.Service) {
	...
	r.POST("/style-profiles", func(c *gin.Context) {
		...
		var req app.StyleProfileRequest
	...
	r.PATCH("/style-profiles/:id/favorite", func(c *gin.Context) {
		...
		var req app.StyleProfileFavoriteRequest
		if err := c.ShouldBindJSON(&req); err != nil {
			fail(c, http.StatusBadRequest, err)
			return
		}
		if err := svc.SetStyleProfileFavorite(user.ID, c.Param("id"), req.Favorite); app.IsProjectNotFound(err) {
			fail(c, http.StatusNotFound, err)
			return
		} else if err != nil {
			failService(c, err)
	...
	r.POST("/style-profiles/:id/use", func(c *gin.Context) {
		...
		if err := svc.TouchStyleProfile(user.ID, c.Param("id")); app.IsProjectNotFound(err) {
			fail(c, http.StatusNotFound, err)
			return
		} else if err != nil {
			failService(c, err)
	...
	r.DELETE("/style-profiles/:id", func(c *gin.Context) {
		...
		if err := svc.DeleteStyleProfile(user.ID, c.Param("id")); app.IsProjectNotFound(err) {
			fail(c, http.StatusNotFound, err)
			return
		} else if err != nil {
			failService(c, err)
