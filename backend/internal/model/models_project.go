...
// ProjectFolder 是用户项目列表级目录；与项目内部素材目录分开。
type ProjectFolder struct {
	ID        string    `json:"id" gorm:"primaryKey;size:36"`
	UserID    string    `json:"userId" gorm:"index;size:36;uniqueIndex:idx_project_folders_user_name,priority:1"`
	ParentID  string    `json:"parentId,omitempty" gorm:"index;size:36"`
	Name      string    `json:"name" gorm:"size:160;uniqueIndex:idx_project_folders_user_name,priority:2"`
	CreatedAt time.Time `json:"createdAt"`
	UpdatedAt time.Time `json:"updatedAt"`
}
