...
type ChannelModel struct {
	...
	// Variants map a supported option combination to an upstream model key.
...
// ChannelModelVariant maps a supported option combination to an executable upstream SKU.
...
type ChannelModelVariant struct {
	ID               string            `json:"id" gorm:"primaryKey;size:36"`
	ChannelModelID   string            `json:"channelModelId" gorm:"size:36;index;uniqueIndex:idx_channel_model_variant_active,priority:1,where:deleted_at IS NULL"`
	SelectorKey      string            `json:"selectorKey" gorm:"size:500;not null;default:{};uniqueIndex:idx_channel_model_variant_active,priority:2,where:deleted_at IS NULL"`
