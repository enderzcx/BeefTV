...
// CreationConfigSignature captures only execution-relevant configuration,
// never channel credentials or retired hosted configuration.
func (r *Repository) CreationConfigSignature(task *model.Task, channelID, modelKey string) (string, error) {
	...
	values["channelModel"] = map[string]any{
		"id": cm.ID, "channelId": cm.ChannelID, "modelKey": cm.ModelKey,
		"providerModelKey": cm.ProviderModelKey, "capability": cm.Capability,
		"protocol": cm.Protocol, "capabilityConfig": cm.CapabilityConfigJSON,
		"capabilityVersion": cm.CapabilityVersion, "enabled": cm.Enabled,
	...
	if task.LogicalModelID != "" {
		...
		values["logical"] = map[string]any{
			"id": logical.ID, "activeRevisionId": logical.ActiveRevisionID,
			"enabled": logical.Enabled, "updatedAt": logical.UpdatedAt,
		}
		var route model.LogicalModelRoute
		if err := db.First(&route, "id = ?", task.RouteID).Error; err != nil {
			return "", err
		}
		values["route"] = map[string]any{
			"id": route.ID, "logicalModelRevisionId": route.LogicalModelRevisionID,
			"channelModelId": route.ChannelModelID, "enabled": route.Enabled,
			"priority": route.Priority, "weight": route.Weight, "updatedAt": route.UpdatedAt,
		}
	}
