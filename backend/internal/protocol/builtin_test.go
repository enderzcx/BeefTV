...
func TestAsyncMediaPollUsesNestedGenerationStatusAndMetadataURL(t *testing.T) {
	adapter, _ := Builtins().Get("minimax-video")
	polled, err := adapter.ParsePoll(context.Background(), PollContext{TaskID: "beef-task-1"}, []byte(`{"id":"beef-task-1","status":"in_progress","progress":100,"metadata":{"generation_status":"completed","stage":"saving","url":"https://cdn.example/beef-result.mp4"}}`))
	if err != nil {
		t.Fatal(err)
	}
	if polled.Status != StatusSucceeded || polled.Result == nil || len(polled.Result.Videos) != 1 || polled.Result.Videos[0].URL != "https://cdn.example/beef-result.mp4" {
		t.Fatalf("polled = %#v", polled)
	}
}
...
func TestImageResponseKeepsBase64AsDataURL(t *testing.T) {
	adapter := officialPackageAdapter(t, "openai-images.beeftv-plugin", "openai-image")
...
func TestOpenAIImagesEditUsesMultipartInlineImageReferences(t *testing.T) {
	adapter := officialPackageAdapter(t, "openai-images.beeftv-plugin", "openai-image")
	if adapter.Metadata().RequiresPublicMediaURLs {
		t.Fatal("OpenAI Images edits must keep local resources inline for multipart upload")
	}
	spec, err := adapter.BuildCreate(context.Background(), RequestContext{Request: GenerationRequest{
		Model:  "gpt-image-2.5",
		Prompt: "combine both references",
		Images: []MediaReference{
			{DataURL: "data:image/png;base64,b25l", MIMEType: "image/png", Role: "edit_source", Order: 0},
			{DataURL: "data:image/png;base64,dHdv", MIMEType: "image/png", Role: "edit_source", Order: 1},
			{DataURL: "data:image/png;base64,bWFzaw==", MIMEType: "image/png", Role: "mask", Order: 2},
		},
	}})
	if err != nil {
		t.Fatal(err)
	...
	for index, want := range []string{"image", "image", "mask"} {
		if spec.Files[index].Name != want || spec.Files[index].Reference.DataURL == "" {
			t.Fatalf("files[%d] = %#v, want inline %q part", index, spec.Files[index], want)
		}
	}
...
func TestAsyncMediaPollKeepsResultKind(t *testing.T) {
	cases := []struct {
		...
		{"volcengine-jimeng-image.beeftv-plugin", "volcengine-jimeng-image", `{"data":{"status":"completed","image_urls":["https://cdn.example/image.png"]}}`, "image"},
		{"async-audio.beeftv-plugin", "async-audio", `{"id":"audio-1","status":"completed","audio_url":"https://cdn.example/audio.mp3"}`, "audio"},
