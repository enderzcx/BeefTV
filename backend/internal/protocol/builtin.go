...
func parseAsyncMediaPoll(c PollContext, payload map[string]any, capability Capability) (PollResult, error) {
	...
	// BeefAPI keeps the transport task in `in_progress` while its nested
	// generation has already completed and is saving the output.  The actual
	// terminal state and media URL live under metadata; prefer that state and
	// flatten the metadata so the normal result extraction can consume `url`.
	if metadata := object(payload["metadata"]); metadata != nil {
		generationStatus := normalizeStatus(firstString(metadata, "generation_status", "generationStatus"))
		payload = mergeMaps(metadata, payload)
		if generationStatus != "" {
