...
func TestManifestUsesOnlyUnifiedTopLevelContract(t *testing.T) {
	...
	canvasOnly := []byte(`{"apiVersion":"beeftv.plugin/v1","id":"canvas-extension","name":"Canvas Extension","version":"1.0.0","contributes":{"canvasNodes":[{"id":"canvas-extension/node","label":"Node","defaultTitle":"Node","defaultSize":{"width":320,"height":200},"schema":{},"renderer":"declarative"}]}}`)
	if _, err := LoadManifest(canvasOnly); err != nil {
		t.Fatalf("providerless unified plugin was rejected: %v", err)
	}
	multi := []byte(`{"apiVersion":"beeftv.plugin/v1","id":"multi-provider","name":"Multi Provider","version":"1.0.0","contributes":{"providers":[{"id":"multi-text","label":"Text","capabilities":["text"],"scopes":["canvas"],"create":{"method":"POST","path":"/text"},"response":{}},{"id":"multi-image","label":"Image","capabilities":["image"],"scopes":["canvas"],"create":{"method":"POST","path":"/image"},"response":{}}]}}`)
