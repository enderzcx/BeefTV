...
func TestDuplicateSystemChannelCopiesSecretsModelsAndVariants(t *testing.T) {
	...
	if len(models[0].Variants) != 1 || models[0].Variants[0].ID == sourceTier.ID || models[0].Variants[0].ProviderModelKey != sourceTier.ProviderModelKey {
		t.Fatalf("copied model variants = %#v", models[0].Variants)
	}
}
