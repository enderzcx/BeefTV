...
func (s *Service) CreateProjectFolder(userID string, req CreateProjectFolderRequest) (model.ProjectFolder, error) {
	...
	if name == "" {
		return model.ProjectFolder{}, BadAuthRequest("文件夹名称不能为空")
	}
	now := time.Now()
	folder := model.ProjectFolder{ID: newID(), UserID: userID, ParentID: strings.TrimSpace(req.ParentID), Name: name, CreatedAt: now, UpdatedAt: now}
...
func (s *Service) DuplicateProject(userID, projectID string) (model.Project, error) {
	source, err := s.repo.ProjectForUser(userID, projectID)
	if err != nil {
