...
type CreationSubmissionOutput struct {
	...
	Execution CreationExecution `json:"execution"`
}
...
func creationError(err error) error {
	...
	if errors.Is(err, repository.ErrLogicalModelUnavailable) {
		return creationConflict("模型已更新或停用，请重新准备执行项")
	}
...
func creationSubmissionOutput(item model.CreationSubmission) CreationSubmissionOutput {
	var execution CreationExecution
	_ = json.Unmarshal([]byte(item.ExecutionJSON), &execution)
	return CreationSubmissionOutput{item, execution}
}
	...
	err := s.repo.MutateCreationRun(userID, id, func(run *model.CreationRun, repo *repository.Repository) error {
		...
		if action == "claim" {
			...
			case "save":
				...
				if !strings.Contains("|idle|running|waiting_answer|waiting_proposal|waiting_canvas|waiting_execution|waiting_task|paused|completed|cancelled|", "|"+req.Status+"|") || req.Status == "" {
...
func (s *Service) prepareCreationTask(userID string, req CreateTaskRequest) (*model.Task, string, error) {
	...
	for _, key := range []string{"channelId", "channelModelKey", "model", "variantId", "apiFormat", "interfaceType", "size", "quality", "transparentBackground", "count", "videoSeconds", "vquality", "videoGenerateAudio", "videoWatermark", "videoArkPrivateAssetUpload", "systemPrompt"} {
...
func creationExecutionFor(task *model.Task, signature string) CreationExecution {
	...
	execution.Options = map[string]any{}
	for _, key := range []string{"size", "videoSeconds", "vquality", "quality", "maxTokens"} {
		if value, ok := config[key]; ok {
			execution.Options[key] = value
		}
	}
	execution.ConfigHash = creationHash([]any{signature, execution.Options})
	return execution
}
...
func (s *Service) PrepareCreationSubmission(userID, id string, req CreationRequest) (*CreationSubmissionOutput, error) {
	...
	err = s.repo.MutateCreationRun(userID, id, func(current *model.CreationRun, repo *repository.Repository) error {
		...
		if e = s.validateCreationStorage(repo, userID, false, int64(len(item.RequestJSON)+len(item.ExecutionJSON)+len(item.ConfigSignature))); e != nil {
			return e
		}
...
func (s *Service) ApproveCreationSubmissions(userID, id string, req CreationRequest) (map[string]any, error) {
	...
	// Resolve execution configuration before the write transaction; the
	// transaction verifies its fingerprint again before admitting work.
	...
	for _, sid := range req.SubmissionIDs {
		...
		if creationExecutionFor(task, sig).ConfigHash != creationSubmissionOutput(*item).Execution.ConfigHash {
			return nil, creationConflict("执行配置已变化，请重新准备并确认")
		}
		prepared[sid] = task
		signatures[sid] = sig
	}
	...
	err := s.repo.MutateCreationRun(userID, id, func(run *model.CreationRun, repo *repository.Repository) error {
		...
		for _, sid := range req.SubmissionIDs {
			...
			if item.RevokedAt != nil {
				return creationConflict("执行项已撤销")
			}
			var request CreateTaskRequest
			_ = json.Unmarshal([]byte(item.RequestJSON), &request)
...
func checkCreationConfigSignature(repo *repository.Repository, task *model.Task, want string) error {
	...
	if got != want {
		return creationConflict("模型执行配置已更新，请重新确认")
	}
	return nil
}
func (s *Service) ExecuteCreationSubmission(userID, id string, req CreationRequest) (*model.Task, error) {
	...
	err = s.repo.MutateCreationRun(userID, id, func(current *model.CreationRun, repo *repository.Repository) error {
		...
		if fresh.ApprovedAt == nil || fresh.RevokedAt != nil {
			return creationConflict("任务尚未批准或执行项已撤销")
		}
