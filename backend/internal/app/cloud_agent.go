...
// reuses worker leases, cancellation and text replay.
type CloudAgentRequest struct {
	...
	// ConversationID is the durable Agent identity. CanvasID is optional and
	// only required when the turn requests canvas context/tool mutations.
...
func validateCloudAgentRequest(req *CloudAgentRequest) error {
	...
	// Backward-compatible callers predating conversationId use the canvas as
	// their durable conversation identity.
	if strings.TrimSpace(req.ConversationID) == "" && strings.TrimSpace(req.CanvasID) != "" {
		req.ConversationID = req.CanvasID
	}
	if err := validateCloudAgentID(req.ConversationID, "会话 ID", 120); err != nil {
		return err
	}
...
const localAgentManagedChannelID = "local"

func (s *Service) validateCloudAgentRequestForService(req *CloudAgentRequest) error {
	if err := validateCloudAgentRequest(req); err == nil {
		return nil
	} else if req == nil || req.LogicalModelID != "" || req.ChannelID != "" || req.ChannelModelKey != "" {
		return err
	}
	if _, ok := s.localAgentProviderConfig(req.Model); !ok {
		return validateCloudAgentRequest(req)
	}
	req.ChannelID = localAgentManagedChannelID
	req.ChannelModelKey = req.Model
	return validateCloudAgentRequest(req)
}
...
func (s *Service) CreateCloudAgentRun(userID string, req CloudAgentRequest, parentID string) (*CloudAgentRun, error) {
	if err := s.validateCloudAgentRequestForService(&req); err != nil {
		return nil, err
	}
	if userID == "" {
		return nil, kernel.Unauthorized("请先登录")
	}
	// Chat-only turns use a synthetic project key so the existing durable task
	// machinery remains unchanged. Real canvas turns retain strict ownership.
	canvasID := req.CanvasID
	chatOnly := canvasID == ""
	if chatOnly {
		canvasID = "agent-conversation:" + req.ConversationID
		req.CanvasID = canvasID
	}
	canvas, err := s.repo.CanvasProjectForUser(userID, canvasID)
	if err != nil && (!chatOnly || !errors.Is(err, gorm.ErrRecordNotFound)) {
		return nil, err
	}
	if chatOnly {
		canvas = nil
	...
	profile, err := s.cloudAgentProfileSnapshot(userID, func() string {
		if canvas != nil {
			return req.CanvasID
		}
		return ""
	...
	// Local callers omit the browser-owned profile revision; when a revision is
	// supplied, preserve the normal optimistic-concurrency check.
	...
	if localConfig, ok := s.localAgentProviderConfig(req.Model); ok && (req.ChannelID == "" || req.ChannelID == localAgentManagedChannelID) && req.LogicalModelID == "" {
	...
	task, err := s.CreateTask(userID, CreateTaskRequest{ProjectID: req.CanvasID, Type: "canvas_text", Operation: cloudAgentOperation, Prompt: req.Prompt, Model: req.Model, LogicalModelID: req.LogicalModelID, Input: input,
		admission: &taskAdmission{ID: id}})
	if err != nil {
		// A concurrent identical request may have won the transaction. Never
		// replace its result or start a duplicate provider task.
