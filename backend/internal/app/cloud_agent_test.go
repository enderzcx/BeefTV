...
func TestCloudAgentValidation(t *testing.T) {
	for _, mutate := range []func(*CloudAgentRequest){
		...
		func(r *CloudAgentRequest) { r.Budget.MaxVideoSeconds = -1 },
...
func TestCloudAgentAdmissionNeedsNoCommerceSchema(t *testing.T) {
	s, db, _, _ := creationTestService(t)
	if err := db.Create(&model.CanvasProject{ID: "agent-canvas", UserID: "user", PayloadJSON: `{"nodes":[]}`}).Error; err != nil {
		t.Fatal(err)
	}
	...
	if err != nil {
		t.Fatalf("local Agent request was rejected: %v", err)
	}
	if run.ID == "" {
		t.Fatal("Agent task was not created")
	}
}
...
func TestCloudAgentConcurrentIdempotencyCreatesOneLocalTask(t *testing.T) {
	s, db, _, _ := creationTestService(t)
	if err := db.Create(&model.CanvasProject{ID: "agent-canvas", UserID: "user", PayloadJSON: `{"nodes":[]}`}).Error; err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Model(&model.Task{}).Count(&taskCount).Error; err != nil || taskCount != 1 {
		t.Fatalf("local Agent tasks = %d, err = %v", taskCount, err)
	}
}
