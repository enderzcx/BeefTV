...
func (h platformHost) AppendAudit(actor *model.User, action, targetType, targetID, summary string, metadata any) error {
	...
	return h.appendAudit(actor, action, targetType, targetID, summary, metadata)
}

func (h platformHost) ChannelConcurrencyLimit(channelID string) (int, error) {
	if h.channelConcurrencyLimit == nil {
		return 0, nil
	}
	return h.channelConcurrencyLimit(channelID)
}

func newPlatformHost(service *Service) platformHost {
	if service == nil {
		return platformHost{}
	}
	return platformHost{
		requireAdmin: service.RequireAdmin,
		appendAudit:  service.appendAdminAudit,
		channelConcurrencyLimit: func(channelID string) (int, error) {
			channel, err := service.repo.SystemChannel(channelID)
			if err != nil {
				return 0, err
			}
...
func (s *Service) platformDomain() *platform.Service {
	...
	if s.platform == nil {
		if s.IsLocalMode() {
			s.platform = platform.NewLocal(s.repo, s.coordinator, newPlatformHost(s))
		} else {
			s.platform = platform.New(s.repo, s.coordinator, newPlatformHost(s))
		}
	}
	return s.platform
}
...
func (s *Service) FeatureAvailability() (*PublicFeatureAvailability, error) {
	if s.IsLocalMode() {
		...
		return platform.ProjectFeatureAvailability(nil, value), nil
	}
	setting, err := s.platformDomain().FeatureAvailability()
	return setting, err
}

func (s *Service) AdminFeatureAvailability(actor *model.User) (*PublicFeatureAvailability, error) {
	setting, err := s.platformDomain().AdminFeatureAvailability(actor)
	if err != nil || !s.IsLocalMode() {
		return setting, err
	}
	return setting, nil
}
...
func (s *Service) FeatureEnabled(feature string) (bool, error) {
	if s.IsLocalMode() {
		...
		case FeatureFrontendModels:
			return false, nil
		case FeatureShortDrama, FeatureTaskCenter, FeatureCustomChannels, FeaturePluginCenter, FeatureSystemPlugins, FeatureTimelineTranscription:
			return true, nil
		}
	}
