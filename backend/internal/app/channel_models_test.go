...
func TestSaveAdminChannelModelPersistsAndPublishesIcon(t *testing.T) {
	...
	publicCatalog := publicChannel(channel, false, []model.ChannelModel{*saved})
	if len(publicCatalog.ModelProfiles) != 1 || publicCatalog.ModelProfiles[0].Icon != "OpenAI" {
		t.Fatalf("public model profiles = %#v", publicCatalog.ModelProfiles)
	}
}
...
func TestSanitizeChannelModelAvailabilityIsIndependentOfVariants(t *testing.T) {
	svc := &Service{}
	...
	if !public.Available {
		t.Fatal("an enabled local model must remain available without a model variant")
	}
	...
	if !public.Available {
		t.Fatalf("model variants changed local availability: %#v", public)
	}
}
